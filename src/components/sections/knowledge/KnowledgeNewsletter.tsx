...
interface KnowledgeNewsletterProps {
  initialProfile?: Profile | null;
}

export function KnowledgeNewsletter({ initialProfile }: KnowledgeNewsletterProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const success = await createLead({
      ...
      source: 'knowledge_newsletter',
      message: 'Đăng ký nhận bản tin & Bản đồ Năng Lượng Lãnh đạo 2026'
    });
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
            ...
            <div className="w-full lg:w-3/5 relative z-10">
              <SlideIn direction="right">
                ...
                {isSubmitted ? (
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-8 text-center flex flex-col items-center gap-4 max-w-lg">
                    <CheckCircle2 className="w-12 h-12 text-green-400" />
                    <div>
                      <h3 className="text-lg font-bold text-white">Đăng ký nhận tài liệu thành công!</h3>
                      <p className="text-white/60 text-sm mt-1">Chúng tôi đã gửi file Bản đồ Năng Lượng Lãnh đạo 2026 tới email của bạn. Hãy kiểm tra hộp thư nhé!</p>
                    </div>
                    <Button variant="secondary" className="px-6 h-10 text-xs font-bold" onClick={() => setIsSubmitted(false)}>
                      Đăng ký email khác
                    </Button>
                  </div>
                ) : (
                  <form className="space-y-4 max-w-lg" onSubmit={handleSubmit}>
                    <div className="flex flex-col sm:flex-row gap-4">
                      <div className="w-full sm:w-1/2">
                        <input 
                          type="text" 
                          className="w-full bg-white/10 border border-white/20 focus:border-cyan-azure focus:ring-4 focus:ring-cyan-azure/20 rounded-xl px-5 h-14 transition-all outline-none text-white placeholder-white/50 font-medium" 
                    ...
                    <Button 
                      ...
                      className="w-full h-14 text-lg font-bold rounded-xl shadow-[0_0_30px_rgba(255,107,0,0.3)] hover:-translate-y-1 hover:shadow-[0_0_40px_rgba(255,107,0,0.5)] transition-all duration-300"
                    >
                      {isSubmitting ? 'ĐANG GỬI...' : 'ĐĂNG KÝ & NHẬN BẢN ĐỒ MIỄN PHÍ'}
                    </Button>
                  ...
                  {strategyCoverUrl ? (
                    <div className="absolute inset-0 rounded-2xl overflow-hidden shadow-2xl border border-white/10">
                      <Image
                        ...
                        alt="Bản đồ Năng Lượng Lãnh đạo"
                        fill
